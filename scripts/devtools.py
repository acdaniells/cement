...
class CementDevtoolsController(Controller):
    class Meta:
        ...
        arguments = [
            (
                ["-y, --noprompt"],
                dict(
                    help="answer yes to prompts.", action="store_true", dest="noprompt"
                ),
            ),
            (
                ["--ignore-errors"],
                dict(
                    help="don't stop operations because of errors",
                    action="store_true",
                    dest="ignore_errors",
                ),
            ),
            (
                ["--loud"],
                dict(help="add more verbose output", action="store_true", dest="loud"),
            ),
        ]
    ...
    def _do_git(self):
        ...
        out, err, res = shell.exec_cmd(["git", "tag", "-a", "-m", VERSION, VERSION])
        if res > 0:
