    ...
    with ThisApp("test-app", config_files=None) as app:
        ...
        if platform.system().lower() in ["windows"]:
            files = [
                ...
                os.path.join(
                    user_home, ".%s" % app.label, "config", "%s.conf" % app.label
                ),
            ]
        else:
            files = [
...
def test_register_hooks_via_meta():
    ...
    app = TestApp(
        define_hooks=["my_custom_hook"], hooks=[("my_custom_hook", my_custom_hook_func)]
    )
    with app:
...
def test_register_hooks_via_meta_retry():
    ...
    app = TestApp(
        extensions=["watchdog"], hooks=[("watchdog_pre_start", my_custom_hook_func)]
    )
    with app:
...
def test_register_handlers_via_meta():
    ...
    app = TestApp(interfaces=[MyTestInterface], handlers=[MyTestHandler])
    with app:
