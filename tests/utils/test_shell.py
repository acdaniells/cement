...
def test_prompt_clear():
    ...
    with mock.patch(INPUT, return_value="Test Input"):
        p = shell.Prompt("Test Prompt", clear=True, clear_command="true")
        assert p.input == "Test Input"
...
def test_prompt_numbered_options():
    ...
    with mock.patch(INPUT, return_value="3"):
        p = shell.Prompt("Test Prompt", options=["yes", "no", "maybe"], numbered=True)
        assert p.input == "maybe"

    # test default value
    with mock.patch(INPUT, return_value=""):
        p = shell.Prompt(
            "Test Prompt", options=["yes", "no", "maybe"], numbered=True, default="2"
        )
        assert p.input == "no"
...
def test_prompt_input_is_none():
    ...
    with mock.patch(INPUT, return_value=""):
        p = shell.Prompt("Test Prompt", max_attempts=3, max_attempts_exception=False)
...
def test_prompt_max_attempts():
    ...
    with mock.patch(INPUT, return_value=""):
        ...
        with raises(FrameworkError, match=msg):
            shell.Prompt("Test Prompt", max_attempts=3, max_attempts_exception=True)
...
def test_prompt_case_insensitive():
    with mock.patch(INPUT, return_value="NO"):
        p = shell.Prompt(
            "Test Prompt", options=["yes", "no", "maybe"], case_insensitive=True
        )
        assert p.input == "NO"
