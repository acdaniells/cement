...
class Tmp:
    ...
    def __init__(self, **kwargs):
        ...
        self.dir = tempfile.mkdtemp(suffix=suffix, prefix=prefix, dir=dir)
        _, self.file = tempfile.mkstemp(suffix=suffix, prefix=prefix, dir=dir)
...
def backup(path, suffix=".bak"):
    ...
    while True:
        if os.path.exists(path):
            if count == -1:
                new_path = "{}{}".format(path, suffix)
            else:
                new_path = "{}{}.{}".format(path, suffix, count)
