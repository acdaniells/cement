...
class HandlerManager:
    ...
    def get(self, interface, handler_label, fallback=None, **kwargs):
        ...
        if interface not in self.app.interface.list():
            raise exc.InterfaceError("Interface '%s' does not exist!" % interface)
        ...
        if handler_label in self.__handlers__[interface]:
            ...
            raise exc.InterfaceError(
                "handlers['{}']['{}'] does not exist!".format(interface, handler_label)
            )
    ...
    def register(self, handler_class, force=False):
        ...
        if not issubclass(handler_class, Handler):
            raise exc.InterfaceError(
                "Class %s " % handler_class + "does not implement Handler"
            )
        ...
        LOG.debug(
            "registering handler '%s' into handlers['%s']['%s']"
            % (handler_class, interface, obj._meta.label)
        )

        if interface not in self.app.interface.list():
            raise exc.InterfaceError(
                "Handler interface '%s' doesn't exist." % interface
            )
        elif interface not in self.__handlers__.keys():
            self.__handlers__[interface] = {}

        if (
            obj._meta.label in self.__handlers__[interface]
            and self.__handlers__[interface][obj._meta.label] != handler_class
        ):

            if force is True:
                LOG.debug(
                    "handlers['{}']['{}'] already exists".format(
        ...
        if not issubclass(handler_class, interface_class):
            raise exc.InterfaceError(
                "Handler %s " % handler_class.__name__
                + "does not sub-class %s" % interface_class.__name__
            )
    ...
    def registered(self, interface, handler_label):
        ...
        if interface in self.app.interface.list():
            if (
                interface in self.__handlers__.keys()
                and handler_label in self.__handlers__[interface]
            ):
                return True
    ...
    def resolve(self, interface, handler_def, **kwargs):
        ...
        if meta_defaults is None:
            ...
            if type(handler_def) == str:
                _meta_label = "{}.{}".format(interface, handler_def)
                meta_defaults = self.app._meta.meta_defaults.get(_meta_label, {})
            elif hasattr(handler_def, "Meta"):
                _meta_label = "{}.{}".format(interface, handler_def.Meta.label)
                meta_defaults = self.app._meta.meta_defaults.get(_meta_label, {})
        ...
        msg = "Unable to resolve handler '{}' of interface '{}'".format(
