...
class Environment:
    ...
    def __init__(self, **kw):
        ...
        try:
            ...
            raise exc.FrameworkError("Daemon user '%s' doesn't exist." % self.user)

        try:
            self.group = kw.get("group", grp.getgrgid(self.user.pw_gid).gr_name)
            self.group = grp.getgrnam(self.group)
        except KeyError:
            raise exc.FrameworkError("Daemon group '%s' doesn't exist." % self.group)
    ...
    def _write_pid_file(self):
        ...
        """
        pid = str(os.getpid())
        LOG.debug("writing pid ({}) out to {}".format(pid, self.pid_file))
    ...
    def switch(self):
        ...
        LOG.debug(
            "setting process uid(%s) and gid(%s)"
        ...
        if self.pid_file and os.path.exists(self.pid_file):
            raise exc.FrameworkError("Process already running (%s)" % self.pid_file)
        else:
...
def daemonize():  # pragma: no cover
    ...
    if "--daemon" in app.argv or hasattr(app, "pargs") and app.pargs.daemon is True:
...
def extend_app(app):
    ...
    app.args.add_argument(
        "--daemon", dest="daemon", action="store_true", help="daemonize the process"
    )
